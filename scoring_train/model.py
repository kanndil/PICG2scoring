...
from models import resnet, resnet2p1d, pre_act_resnet, wide_resnet, resnext, densenet, vit
...
def generate_model(opt):
    assert opt.model in [
        'resnet', 'resnet2p1d', 'preresnet', 'wideresnet', 'resnext', 'densenet', 'vit'
    ]
    ...
    if opt.model == 'resnet':
        ...
        model = vit.ViT(image_size = opt.sample_size,          # image size
                        frames = opt.sample_duration,               # number of frames
...
def load_pretrained_model(model, pretrain_path):
    if pretrain_path:
        ...
        pretrain_dict = {k: v for k, v in new_state_dict.items() if k in model_dict.keys()}
        # k 是每一层的名称，v是权重数值
        ...
        # for k, v in pretrain['state_dict'].items():
        #     name = k[7:]  # remove `module.`，表面从第7个key值字符取到最后一个字符，正好去掉了module.
        #     if name.split('.')[0] != 'conv1' and name.split('.')[0] != 'bn1' and name.split('.')[0] != 'fc':
        #         new_state_dict[name] = v  # 新字典的key值对应的value为一一对应的值。
