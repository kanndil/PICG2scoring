...
INSTRUCT = {
    "T2W": "...",  # use the same instruct text as you provided
    "non-T2W": "..."
}
INPUT = "Describe the apperance of this MRI image."
tf = torch.nn.Upsample(size=(84, 84, 98))
...
def process_data(json_path, output_root):
    with open(json_path) as f:
        data = json.load(f)

    cnter = 0
    for data_item in tqdm(data):
        instruct_key = 'T2W' if not (cnter + 1) % 3 else 'non-T2W'
        instruct = INSTRUCT[instruct_key]
        prompt = llama.format_prompt(instruct + INPUT)
...
process_data("prostate_public_train.json", "prostate/feature_input60/train")
process_data("prostate_public_val.json", "prostate/feature_input60/val")
