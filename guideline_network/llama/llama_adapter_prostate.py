...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
        ...
        # ------------
        # replace conv1 with 3D conv
        conv1 = self.clip.visual.conv1
    ...
    def clip_encode_image(self, x):
        # modified from CLIP
        #<left>t2w<\left> <right>adc<\right>#
        # t2w
        x = self.clip.visual.conv1(x)  # shape = [*, width, grid, grid]
        # shape = [*, width, grid ** 2]
        x = x.reshape(x.shape[0], x.shape[1], -1)
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        # t2w = torch.cat((t2w, t2w, t2w), dim=1)
        # adc = torch.cat((adc, adc, adc), dim=1)

        # x1 = self.clip.visual.conv1(t2w)  # shape = [*, width, grid, grid]
        # # shape = [*, width, grid ** 2]
        # x1 = x1.reshape(x1.shape[0], x1.shape[1], -1)
        # x1 = x1.permute(0, 2, 1)  # shape = [*, grid ** 2, width]

        # x2 = self.clip.visual.conv1(adc)  # shape = [*, width, grid, grid]
        # # shape = [*, width, grid ** 2]
        # x2 = x2.reshape(x2.shape[0], x2.shape[1], -1)
        # x2 = x2.permute(0, 2, 1)  # shape = [*, grid ** 2, width]        

        # --------------------------
        # fill in 252 patches to 256
        # x = torch.cat([x, *[torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device) for i_ in range(4)]], dim=1)
        p = torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
        x = torch.cat([p, p, x, p, p], dim=1)
        # --------------------------
    ...
    def forward(self, tokens, labels, imgs):
        # ----------
        # # t2w, dwi, adc: (1, 84, 84, 84)
        # imgs = torch.cat((t2w, dwi, adc), dim=1) # (3, 84, 84, 84)
        # ----------
        imgs = torch.cat((imgs, imgs, imgs), dim=1)  
        visual_query = self.forward_visual(imgs)

        _bsz, seqlen = tokens.shape
        h = self.llama.tok_embeddings(tokens.cuda().long())
        ...
        if labels.sum() == 0:
            ...
            c_loss = self.criterion(output.reshape(-1, self.llama.vocab_size), labels.cuda().flatten())
    ...
    def forward_inference(self, visual_query, tokens, start_pos: int):
        ...
        for layer in self.llama.layers[-1 * self.query_layer:]:
            ...
            adapter_index = adapter_index + 1
            # print(adapter_index) # 31
            # if adapter_index == 15:
            #     h_feature = h

        h = self.llama.norm(h)
        # print("h:", h.shape)
        # print("h_feature:", h_feature.shape)
        # print("h1:", h[:, -1, :].shape)

        output = self.llama.output(h[:, -1, :])

        return output.float(), h.float()
    ...
    def generate(
        ...
        prev_pos = 0
        save_middle_feature = []
        # print("start_pos, total_len:",start_pos, total_len)
        for cur_pos in range(start_pos, total_len):
            # print("cur_pos:", cur_pos)
            with torch.cuda.amp.autocast():
                logits, middle_feature = self.forward_inference(visual_query, tokens[:, prev_pos:cur_pos], prev_pos)
                # print("logits:", logits.shape)
                # print("middle_feature:", middle_feature.shape)
                save_middle_feature.append(middle_feature)
            if temperature > 0:
                probs = torch.softmax(logits / temperature, dim=-1)
                # print("prob:", probs.shape)
                next_token = sample_top_p(probs, top_p)
                # print("next_token1:", next_token.shape)
            else:
                next_token = torch.argmax(logits, dim=-1)
            # print("next_token2:", next_token.shape)
            next_token = next_token.reshape(-1)
            # print("next_token3:", next_token.shape)

            next_token = torch.where(
                input_text_mask[:, cur_pos], tokens[:, cur_pos], next_token
            )
            # print("next_token4:", next_token.shape)
        ...
        return decoded, save_middle_feature
